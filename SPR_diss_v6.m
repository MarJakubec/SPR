...
%RUSdis = Slt/(1-Slt)*(RUsmax-RUSdiss)
%RUSmax - initial dissociation state
diss_time = 200;
for o = 1:size(to_plotX,2)
to_plotX1 = to_plotX(:,o)-diss_time;

%extract intensity for each time
if isempty(CH_del{o})
to_anal = to_plotY{o}(:,4:15);
else
A = CH_del{o} + 3;
j_del = setdiff([4:15],A,'stable');    
to_anal = to_plotY{o}(:,j_del);
end
% % moving to zero
...
xlim([-100 350])
ylim([-Inf max(max(to_anal))*1.1])

subplot(2,2,2)
hold on 
for i = 1:200  % dp pnlu first 200 seconds to_plotX1(end)
...
alpha(o) =  fitresult.alpha;
beta(o) = fitresult.beta;
koffa(o) = fitresult.koffa;
koffb(o) = fitresult.koffb;
Sret(o) = fitresult.Sret;
%calculation mediam koff
% koff = (alpha*koffa + beta*koffb)/(alpha+beta)
koff(o) = (alpha(o)*koffa(o)+beta(o)*koffb(o))/(alpha(o)+beta(o));

dim2 = [.75 .05 .3 .3];
str2 = {sprintf('K_{off} = %0.4f s^{-1}',koff(o))};
...
print(sprintf('%s %s Channel %d Diss fit.png',name,lipid,o+1),'-dpng')
close all 
Sl_exp{o} = Sl;
Sl_fit_exp{o} = fitresult(0:200); 
